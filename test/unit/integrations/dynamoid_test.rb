...
Dynamoid.configure do |config|
  ...
  config.access_key = '123132123123'
  config.secret_key = 'some random'
  config.namespace = 'sp'
  config.read_capacity = Integer(10)
  config.write_capacity = Integer(5)
...
module DynamidTest
  class BaseTestCase < Test::Unit::TestCase
    ...
    protected
      # Creates a new Dynamoid model (and the associated table)
      def new_model(table_name = :foo, &block)

        model = Class.new do
          (class << self; self; end).class_eval do
            define_method(:name) { "DynamoidTest::#{table_name.to_s.capitalize}" }
  ...
  class IntegrationTest < BaseTestCase
    def test_should_have_an_integration_name
      assert_equal :dynamoid, StateMachine::Integrations::Dynamoid.integration_name
    end

    def test_should_be_available
      assert StateMachine::Integrations::Dynamoid.available?
    end

    def test_should_match_if_class_includes_dynamoid
      assert StateMachine::Integrations::Dynamoid.matches?(new_model)
    end

    def test_should_not_match_if_class_does_not_include_dynamoid
      assert !StateMachine::Integrations::Dynamoid.matches?(Class.new)
    end

    def test_should_have_defaults
      assert_equal({:action => :save}, StateMachine::Integrations::Dynamoid.defaults)
    end

    def test_should_have_a_locale_path
      assert_not_nil StateMachine::Integrations::Dynamoid.locale_path
    end
  end
  ...
  class MachineWithDifferentColumnDefaultTest < BaseTestCase
    ...
    def test_should_generate_a_warning
      assert_match(/Both DynamoidTest::Foo and its :status machine have defined a different default for "status". Use only one or the other for defining defaults to avoid unexpected behaviors\./, $stderr.string)
    end
  ...
  class MachineWithEventAttributesOnSaveBangTest < BaseTestCase
    ...
    def test_should_fail_if_event_is_invalid
      @record.state_event = 'invalid'
      assert_raise(Dynamoid::Errors::Validations) { @record.save! }
    end
  ...
  class MachineWithScopesAndOwnerSubclassTest < BaseTestCase
    def setup
      ...
      DynamoidTest.const_set('Foo', @model)
      ...
      DynamoidTest.const_set('SubFoo', @subclass)
    end
    ...
    def teardown
      DynamoidTest.send(:remove_const, 'SubFoo')
      DynamoidTest.send(:remove_const, 'Foo')
    end
  end
  ...
  class MachineWithInternationalizationTest < BaseTestCase
    def setup
      ...
      I18n.backend.translate(:en, 'dynamoid.errors.messages.invalid_transition', :event => 'ignite', :value => 'idling')
    ...
    def test_should_use_defaults
      I18n.backend.store_translations(:en, {
        :dynamoid => {:errors => {:messages => {:invalid_transition => 'cannot %{event}'}}}
      })
    ...
    def test_should_allow_customized_error_key
      I18n.backend.store_translations(:en, {
        :dynamoid => {:errors => {:messages => {:bad_transition => 'cannot %{event}'}}}
      })
    ...
    def test_should_allow_customized_state_key_scoped_to_class_and_machine
      I18n.backend.store_translations(:en, {
        :dynamoid => {:state_machines => {:'dynamoid_test/foo' => {:state => {:states => {:parked => 'shutdown'}}}}}
      })
    ...
    def test_should_allow_customized_state_key_scoped_to_class
      I18n.backend.store_translations(:en, {
        :dynamoid => {:state_machines => {:'dynamoid_test/foo' => {:states => {:parked => 'shutdown'}}}}
      })
    ...
    def test_should_allow_customized_state_key_scoped_to_machine
      I18n.backend.store_translations(:en, {
        :dynamoid => {:state_machines => {:state => {:states => {:parked => 'shutdown'}}}}
      })
    ...
    def test_should_allow_customized_state_key_unscoped
      I18n.backend.store_translations(:en, {
        :dynamoid => {:state_machines => {:states => {:parked => 'shutdown'}}}
      })
    ...
    def test_should_support_nil_state_key
      I18n.backend.store_translations(:en, {
        :dynamoid => {:state_machines => {:states => {:nil => 'empty'}}}
      })
    ...
    def test_should_allow_customized_event_key_scoped_to_class_and_machine
      I18n.backend.store_translations(:en, {
        :dynamoid => {:state_machines => {:'dynamoid_test/foo' => {:state => {:events => {:park => 'stop'}}}}}
      })
    ...
    def test_should_allow_customized_event_key_scoped_to_class
      I18n.backend.store_translations(:en, {
        :dynamoid => {:state_machines => {:'dynamoid_test/foo' => {:events => {:park => 'stop'}}}}
      })
    ...
    def test_should_allow_customized_event_key_scoped_to_machine
      I18n.backend.store_translations(:en, {
        :dynamoid => {:state_machines => {:state => {:events => {:park => 'stop'}}}}
      })
    ...
    def test_should_allow_customized_event_key_unscoped
      I18n.backend.store_translations(:en, {
        :dynamoid => {:state_machines => {:events => {:park => 'stop'}}}
      })
    ...
    def test_should_only_add_locale_once_in_load_path
      assert_equal 1, I18n.load_path.select {|path| path =~ %r{dynamoid/locale\.rb$}}.length

      # Create another Dynamoid model that will triger the i18n feature
      new_model
    ...
    def test_should_add_locale_to_beginning_of_load_path
      ...
      default_locale = File.dirname(__FILE__) + '/../../../lib/state_machine/integrations/dynamoid/locale.rb'
