module StateMachine
  module Integrations #:nodoc:
    # Adds support for integrating state machines with Dynamoid models.
    #
    # == Examples
    #
    ...
    # is called.
    #
    # In Dynamoid, these automated events are run in the following order:
    ...
    # caveat here is that, due to a constraint in Dynamoid's validation
    ...
    #
    # Because of the way named scopes work in Dynamoid, they *cannot* be
    # chained.
    #
    ...
    # == Callbacks
    #
    # All before/after transition callbacks defined for Dynamoid models
    # behave in the same way that other Dynamoid callbacks behave.  The
    ...
    # == Observers
    #
    # In addition to support for Dynamoid-like hooks, there is additional support
    # for Dynamoid observers.  Because of the way Dynamoid observers are designed,
    ...
    #
    #   class VehicleObserver < Dynamoid::Observer
    ...
    #
    #   class StateMachineObserver < Dynamoid::Observer
    ...
    # * <tt>dynamoid.state_machines.#{model_name}.#{machine_name}.states.#{state_name}</tt>
    # * <tt>dynamoid.state_machines.#{model_name}.states.#{state_name}</tt>
    # * <tt>dynamoid.state_machines.#{machine_name}.states.#{state_name}</tt>
    # * <tt>dynamoid.state_machines.states.#{state_name}</tt>
    #
    ...
    # * <tt>dynamoid.state_machines.#{model_name}.#{machine_name}.events.#{event_name}</tt>
    # * <tt>dynamoid.state_machines.#{model_name}.events.#{event_name}</tt>
    # * <tt>dynamoid.state_machines.#{machine_name}.events.#{event_name}</tt>
    # * <tt>dynamoid.state_machines.events.#{event_name}</tt>
    #
    ...
    module Dynamoid
      ...
      require 'state_machine/integrations/dynamoid/versions'

      # The default options to use for state machines using this integration
      @defaults = {:action => :save}

      # Classes that include Dynamoid::Document will automatically use the
      # Dynamoid integration.
